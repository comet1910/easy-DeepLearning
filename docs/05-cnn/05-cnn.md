...
![](images/ch06_img07.png)

![](images/ch06_img08.png)

可以看到填充和步幅都会影响输出数据的形状大小。增大填充，输出数据形状大小会变大；增大步幅，输出数据形状大小会变小。让我们分析一下给定填充和步幅如何计算输出数据的形状大小。

假设输入数据形状为$\left( H,W \right)$，卷积核大小为$\left( FH,FW \right)$，填充为$P$，步幅为$S$，输出数据形状为$\left( OH,OW \right)$，可得：
...
例如，对于形状为$\left( 4,4 \right)$的输入数据应用幅度为1的填充，并应用步幅为3，卷积核大小为$\left( 3,3 \right)$的卷积运算：

$$
OH=OW=\frac{4+2-3}{3}+1=2
$$

得到形状为$\left( 2,2 \right)$的输出数据。
...
![](images/ch06_img09.png)

使用1个形状为$\left( C,FH,FW \right)$的卷积核，对形状为$\left( C,H,W \right)$的输入数据进行卷积运算，输出1张形状为$\left( OH,OW \right)$特征图，即输出的通道数为1。

![](images/ch06_img10.png)

若想在通道方向获得多个卷积运算的输出，需要使用多个卷积核。

![](images/ch06_img11.png)

使用$FN$个卷积核，输出特征图也变为$FN$个：

![](images/ch06_img12.png)

若考虑偏置，则：

![](images/ch06_img13.png)
...
![](images/ch06_img14.png)

![](images/ch06_img15.png)
...
为了解决深度网络的梯度消失问题，ResNet以 VGG 为基础，引入了“快捷结构”。这样一来，网络学习的目标就由原始的输出$h\left( x \right)$变为了$h\left( x \right)-x$，这被称为“残差学习”；引入的这个恒等映射被称为“残差连接”（或者“跳跃连接”），这种网络结构也被称为“残差网络”（Residual Network，ResNet）。

![](images/ch06_img22.png)
